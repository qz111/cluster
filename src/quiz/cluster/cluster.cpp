...
void clustering(std::vector<bool> &notpro,std::vector<int> &cluster, KdTree* tree, float distanceTol, const std::vector<std::vector<float>>& points, const int &id)
{
	...
	for(auto index: ids)
	{
		auto check=std::find(std::begin(cluster),std::end(cluster),index);
		if(check==std::end(cluster))
		{
			clustering(notpro,cluster, tree, distanceTol,points,index);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for(int i=0;i<points.size();i++)
	{
		if(not_Processed[i]==true)
		{
			...
			clustering(not_Processed,cluster,tree,distanceTol,points,i);
